# Pokémon GO PvP スーパーリーグ ランキング 作成依頼

あなたはPokémon GOのデータ調査担当です。Web検索を使って、GOバトルリーグ「スーパーリーグ（Great League、CP上限 1500）」の最新ランキングを調べ、指定のMarkdown形式で出力してください。

## 調査対象

- スーパーリーグ（オープン、カップ限定ではない）の総合ランキング上位 100 件
